# astrbot_plugin_video_sense

让 Bot 看懂视频——通过 Gemini 原生多模态视频理解，分析群里分享的视频文件的画面、内容与氛围。

## 功能

- 分析视频文件的画面内容、场景、人物动作和整体氛围
- 消息中的视频文件自动记录，无需引用即可按序号分析
- 支持自动分析模式：收到视频立刻调用 Gemini，结果可注入对话上下文
- 支持指令触发和 LLM 工具自动触发（工具可携带具体追问）
- 指令触发可使用独立的分析提示词
- 本地文件直接读取，远程文件按需下载
- 分析结果缓存，重复分析不消耗 API
- **双传输模式**：小视频（默认 ≤15MB）内嵌传输；大视频自动走官方 Files API（免费层 2GB），突破 20MB 请求体限制
- 每会话缓存数量上限，防止长会话内存膨胀

## 使用方式

### 指令触发

指令需要先唤醒 Bot（群聊中 @ 机器人或使用唤醒词，私聊默认直接触发），然后发送 `/分析视频`，附带或引用视频文件：

```
/分析视频
/分析视频 这个视频在干嘛？
```

> 若你的唤醒词不含 `/`（如 `!`），请发送 `分析视频`（不带斜杠），框架会先剥离唤醒词再匹配指令。

如果没有附带或引用文件，Bot 会列出对话中缓存的视频文件，请用序号指定：

```
/分析视频 1
/分析视频 1 这个视频适合发到朋友圈吗？
```

### LLM 工具自动触发

开启配置中的「启用 LLM 工具调用」后，Bot 提供三个工具：

| 工具 | 说明 |
|------|------|
| `analyze_current_video` | 分析当前消息或引用消息中的视频（可带追问） |
| `list_video_files` | 列出对话中所有缓存的视频文件 |
| `analyze_video_by_number` | 按序号分析指定视频 |

发送视频后与 Bot 对话，Bot 会自动判断是否需要分析。

### 自动分析 + 上下文注入

开启「自动分析视频」后，收到视频文件时会在后台自动调用 Gemini 分析，无需等待指令。

如果同时开启「注入分析结果到对话上下文」，分析结果会以 `[视频感知]` 的形式在下一次 LLM 请求中自动注入，Bot 可直接感知视频内容而无需调用工具。

## 支持的视频格式

mp4、mov、webm、avi、mpeg、mpg、flv、wmv、3gpp（与 [Gemini 官方文档](https://ai.google.dev/gemini-api/docs/video-understanding) 对齐）

## 配置说明

### 添加 API 接入方

插件支持多个接入方，每个接入方对应一个 Gemini API 来源。

**Gemini 官方接口**

| 字段 | 说明 |
|------|------|
| 供应商名称 | 自定义标识，如 `gemini-official` |
| API Key | 在 [Google AI Studio](https://aistudio.google.com/apikey) 获取 |
| 模型名称 | 推荐 `gemini-2.0-flash` 或 `gemini-2.5-flash` |
| 超时时间 | 建议不低于 180 秒 |

**OpenAI 兼容中转站**

| 字段 | 说明 |
|------|------|
| 供应商名称 | 自定义标识 |
| API Key | 中转站提供的访问密钥 |
| Base URL | 中转站地址，如 `https://xxx.com/v1` |
| 模型名称 | 按中转站实际支持的模型填写 |
| 超时时间 | 建议不低于 180 秒 |

> 中转站需支持 Gemini `generateContent` 接口且支持视频输入。

### 分析设置

**分析模型**

格式为 `供应商名称/模型名称`，供应商名称需与上方配置中的名称完全一致（区分大小写）。留空时自动使用第一个接入方。

例如：`gemini-official/gemini-2.0-flash`

**其他配置项**

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| 分析系统提示词 | Gemini 分析指令，自动分析和 LLM 工具触发时使用 | 内置提示词 |
| 指令触发使用不同提示词 | 开启后可单独配置 `/分析视频` 的提示词 | 关闭 |
| 指令触发提示词 | 仅 `/分析视频` 指令触发时使用 | 同分析系统提示词 |
| 启用 LLM 工具调用 | 开启后 Bot 可主动分析 | 开启 |
| 最大文件大小 | 文件总大小上限，超过直接拒绝（Files API 免费层 2GB） | 100 MB |
| 内嵌传输上限 | ≤ 此大小用内嵌传输（请求体含 base64 约膨胀 33%，官方上限 20MB），超过走 Files API | 15 MB |
| 启用 Files API | 超过内嵌上限时通过官方 Files API 上传（仅官方接口，中转站需关闭） | 开启 |
| 每会话缓存视频数上限 | 超出自动丢弃最旧条目 | 50 |
| 支持的视频格式 | 仅列表内格式会被缓存 | mp4/mov/webm/avi/mpeg/mpg/flv/wmv/3gpp |
| 自动分析视频 | 收到本地视频文件时自动分析（远程文件仍会记录，可用指令或工具手动触发下载分析） | 关闭 |
| 注入分析结果到对话上下文 | 自动分析的结果追加到对话上下文（需开启自动分析） | 关闭 |
| 调试模式 | 输出详细日志，方便排查 | 关闭 |

> **Files API 说明**：视频超过「内嵌传输上限」时，插件通过官方 [Files API](https://ai.google.dev/gemini-api/docs/files)（resumable 上传协议）上传后引用分析。免费层单文件上限 2GB。OpenAI 兼容中转站通常不支持 Files API，请关闭「启用 Files API」并将「内嵌传输上限」调高或压缩视频。

## 开发

```bash
pip install pytest pytest-asyncio aiohttp
python -m pytest tests -v
```
